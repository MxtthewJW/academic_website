---
title: <center> **The relationship between physical & social dominance** </center>
...
During a summer internship, I collected data for an experiment investigating the relationship between physical and social dominance under the topic of intrasexual competition. 

This was the first RMarkdown I'd produced. It goes through the data analysis process to test this hypothesis.
...
Firstly, here is a quick overview of each package. `tidyverse` is used for `ggplot2` and reads in our data. The `psych` package is used for our correlation analyses and reliability testing. Both `lme4` and `lmerTest` produce the linear model. For principle compenetns analysis and Bayesian analysis, `pacman` and `BayesFactor` are needed, respectively. 
...
The `summary()` shows us in the proportion of variance row that PC1 accounts for 40% of the variance in the data, PC2 accounts for 13%, PC3 10% and so on... Within this data, there are 10 principle components which explain almost all of the variance in our data. 
