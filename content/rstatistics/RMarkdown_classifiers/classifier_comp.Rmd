---
title: <center> **A comparison of three classifiers** </center>
...
Here, we see the trained data for naive bayes to build its model. For these naive bayes plots, no code is provided as my machine could not render them using RMarkdown (so they are images taken from the R script). However, the code is exactly the same as seen in other plots, just with changes to the labels.
